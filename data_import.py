...
class data_set():                   #circ: circumference
    def __init__(self, folder, sample_set, sample_name, T_anneal):
...
def filename_data_assigner(name, separator="_"):
    ...
    if structure == "Areas":
        L=[0.1, 0.15, 0.2, 0.3, 0.4, 0.5]
        area_nr = int(contents[-1].split(".")[0].split("-")[0])-1
        length = L[area_nr]
        circumference= 4*length
        area=length*length

    if structure == "Stripes":
        L=[0.1, 0.15, 0.2, 0.3, 0.4, 0.5, 0.8, 1.8, 1.2, 2.133]
        contents = name.split(separator) 
        i = len(contents)
        if i==5:
